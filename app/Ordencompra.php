...
class Ordencompra extends Model
{
  use DatesTranslator;
    ...
    {
      $modal='';
      $cotizacion=ContratacionDirecta::find($id);
      $modal.='<div class="col-md-12">
      <div class="panel panel-primary">
        <div class="panel-heading">Registro de orden de compra</div>
        <div class="panel-body">
            ...
            <div class="col-md-6">
              <input type="hidden" name="contratacion_id" value="'.$cotizacion->id.'" id="cotizacion_id">';
              
                $modal.='<textarea name="actividad" rows="3" class="form-control" readonly>'.$cotizacion->nombre.'
                </textarea>';
        ...
        <div class="box-body table-responsive">
            <table class="table table-striped table-bordered table-hover" id="">
                ...
                $modal.='</thead>
                <tbody id="cuerpo">';
                  foreach($cotizacion->materiales as $key => $detalle):
                     $total=$total+$detalle->precio*$detalle->cantidad;
                    $modal.='<tr>
                      <td>'.($key+1).'</td>
                      <td>'.$detalle->material->nombre.'</td>
                      <td>'.$detalle->marca.'</td>
                      <td>'.$detalle->medida->nombre_medida.'</td>
                      <td>'.$detalle->cantidad.'</td>
