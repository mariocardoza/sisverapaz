$(document).ready(function(e){
    ...
    $(document).on("click","#materiales_recibidos", function(e){
      swal({
        ...
        if (result.value) {
          $.ajax({
            url:'../solicitudes/cambiarestado/'+elid,
            type:'PUT',
            dataType:'json',
    ...
    $(document).on("click","#seleccionar",function(e){
      ...
      idsolirequi = $(this).attr('data-solirequi');
      swal({
        title: '¿Está seguro?',
        ...
        if (result.value) {
          ...
          seleccionarr(idcot,idsolirequi);
        } else if (result.dismiss === swal.DismissReason.cancel) {
          swal(
            'Cancelado',
    ...
    $(document).on("click","#registrar_solicitud",function(e){
      ...
      var id=$(this).attr("data-id");
      $.ajax({
        url:'../solicitudes/formulariosoli/'+id,
        type:'get',
        dataType:'json',
        success: function(json){
          if(json[0]==1){
...
$(document).on("click","#cancelar_soli",function(e){
    ...
    //}else{
      $.ajax({
        url:'../solicitudcotizaciones/storer2',
        headers: {'X-CSRF-TOKEN':token},
        type:'post',
        data:{formapago,encargado,cargo,solicitud,unidad,lugar_entrega,fecha_limite,tiempo_entrega,requi},
        success: function(response){
...
function seleccionarr(idcot,idsolirequi)
{
  var ruta ="../cotizaciones/seleccionarrr";
  $.ajax({
    url: ruta,
          type: 'POST',
          data:{idcot,idsolirequi},
...
function info(id){
  modal_cargando();
  $.ajax({
    url:'../solicitudes/informacion/'+id,
    type:'GET',
    data:{},
    success: function(json){
      if(json[0]==1){
